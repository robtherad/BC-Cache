...
waitUntil {if (!isNil "phx_opfText") then {true} else {sleep 5;};};
waitUntil {if (!isNil "phx_bluText") then {true} else {sleep 5;};};
    ...
    if (phx_show_timeUI) then {
        ...
        _display = uiNameSpace getVariable "redforStructText";
            _setText = _display displayCtrl 1001;
                _setText ctrlSetStructuredText (parseText format ["%1",phx_opfText]);
        _display2 = uiNameSpace getVariable "bluforStructText";
            _setText2 = _display2 displayCtrl 1002;
                _setText2 ctrlSetStructuredText (parseText format ["%1",phx_bluText]);
    } else {
